import { useEffect, useState } from 'react'
import type { Session } from '@supabase/supabase-js'
import { CalendarDays, StickyNote, BellRing, Sparkles, LogOut, BellPlus, Check } from 'lucide-react'
import { toast } from 'sonner'
import { supabase } from '@/integrations/supabase/client'
import { ativarNotificacoes, statusNotificacoes } from '@/lib/push'
import AbaAgenda from '@/components/abas/AbaAgenda'
import AbaNotas from '@/components/abas/AbaNotas'
import AbaLembretes from '@/components/abas/AbaLembretes'
import AbaAssistente from '@/components/abas/AbaAssistente'
import {
  DropdownMenu, DropdownMenuContent, DropdownMenuItem,
  DropdownMenuSeparator, DropdownMenuTrigger,
} from '@/components/ui/dropdown-menu'

type Aba = 'agenda' | 'notas' | 'lembretes' | 'assistente'

const ABAS: Array<{ id: Aba; rotulo: string; Icone: typeof CalendarDays }> = [
  { id: 'agenda', rotulo: 'Agenda', Icone: CalendarDays },
  { id: 'notas', rotulo: 'Notas', Icone: StickyNote },
  { id: 'lembretes', rotulo: 'Lembretes', Icone: BellRing },
  { id: 'assistente', rotulo: 'Assistente', Icone: Sparkles },
]

/**
 * Shell único: sidebar fina no desktop, barra inferior no celular (polegar
 * alcança). As abas montam uma vez e ficam vivas escondidas — trocar de aba
 * não perde o rascunho da nota nem recarrega a agenda.
 */
export default function Painel({ sessao }: { sessao: Session }) {
  const [aba, setAba] = useState<Aba>('agenda')
  const [temPush, setTemPush] = useState<boolean | null>(null)

  const nome =
    (sessao.user.user_metadata?.nome as string) ||
    sessao.user.email?.split('@')[0] ||
    'você'

  useEffect(() => {
    statusNotificacoes().then(setTemPush)
  }, [])

  const ligarPush = async () => {
    try {
      await ativarNotificacoes()
      setTemPush(true)
      toast.success('Notificações ativadas neste aparelho!')
    } catch (e: unknown) {
      toast.error((e as Error).message || 'Não consegui ativar as notificações.')
    }
  }

  const sair = () => supabase.auth.signOut()

  return (
    <div className="min-h-screen bg-navy-950 flex">
      {/* Sidebar (desktop) */}
      <aside className="hidden lg:flex flex-col w-60 border-r border-navy-800 bg-navy-900/60 p-4 gap-1">
        <div className="flex items-center gap-2.5 px-2 py-3 mb-3">
          <img src="/icon-diamond.svg" alt="" className="w-8 h-8" />
          <div className="leading-tight">
            <p className="font-black text-white">Meu Auxiliar</p>
            <p className="text-[10px] font-bold uppercase tracking-wider text-primary">by Diamond</p>
          </div>
        </div>
        {ABAS.map(({ id, rotulo, Icone }) => (
          <button
            key={id}
            onClick={() => setAba(id)}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold transition-colors ${
              aba === id
                ? 'bg-primary/15 text-primary'
                : 'text-slate-400 hover:text-white hover:bg-navy-800'
            }`}
          >
            <Icone size={18} />
            {rotulo}
          </button>
        ))}
        <div className="mt-auto">
          {temPush === false && (
            <button
              onClick={ligarPush}
              className="w-full flex items-center gap-2 px-3 py-2 mb-2 rounded-xl text-xs font-bold text-amber-300 bg-amber-500/10 hover:bg-amber-500/20"
            >
              <BellPlus size={15} /> Ativar notificações
            </button>
          )}
          <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-navy-800/60">
            <div className="min-w-0">
              <p className="text-xs font-bold text-white truncate">{nome}</p>
              <p className="text-[10px] text-slate-500 truncate">{sessao.user.email}</p>
            </div>
            <button onClick={sair} title="Sair" className="text-slate-400 hover:text-danger p-1.5">
              <LogOut size={16} />
            </button>
          </div>
        </div>
      </aside>

      {/* Conteúdo */}
      <div className="flex-1 flex flex-col min-w-0">
        {/* Topo (celular) */}
        <header className="lg:hidden pt-safe sticky top-0 z-20 bg-navy-950/90 backdrop-blur border-b border-navy-800">
          <div className="flex items-center justify-between px-4 h-14">
            <div className="flex items-center gap-2">
              <img src="/icon-diamond.svg" alt="" className="w-7 h-7" />
              <span className="font-black text-white">Meu Auxiliar</span>
            </div>
            <DropdownMenu>
              <DropdownMenuTrigger className="w-9 h-9 rounded-full bg-primary/20 text-primary font-black grid place-items-center">
                {nome.charAt(0).toUpperCase()}
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="bg-navy-900 border-navy-700 text-white">
                <div className="px-2 py-1.5">
                  <p className="text-sm font-bold">{nome}</p>
                  <p className="text-xs text-slate-400">{sessao.user.email}</p>
                </div>
                <DropdownMenuSeparator className="bg-navy-700" />
                <DropdownMenuItem onClick={ligarPush} disabled={temPush === true} className="gap-2">
                  {temPush ? <Check size={15} className="text-success" /> : <BellPlus size={15} />}
                  {temPush ? 'Notificações ativas' : 'Ativar notificações'}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={sair} className="gap-2 text-danger focus:text-danger">
                  <LogOut size={15} /> Sair
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </header>

        {/* Abas vivas: esconder com hidden preserva estado (rascunho, mês aberto) */}
        <main className="flex-1 min-h-0 pb-20 lg:pb-0">
          <div hidden={aba !== 'agenda'} className="h-full"><AbaAgenda ativa={aba === 'agenda'} /></div>
          <div hidden={aba !== 'notas'} className="h-full"><AbaNotas ativa={aba === 'notas'} /></div>
          <div hidden={aba !== 'lembretes'} className="h-full"><AbaLembretes ativa={aba === 'lembretes'} /></div>
          <div hidden={aba !== 'assistente'} className="h-full">
            <AbaAssistente ativa={aba === 'assistente'} nome={nome} />
          </div>
        </main>

        {/* Barra inferior (celular) */}
        <nav className="lg:hidden fixed bottom-0 inset-x-0 z-20 bg-navy-900/95 backdrop-blur border-t border-navy-800 pb-safe">
          <div className="grid grid-cols-4">
            {ABAS.map(({ id, rotulo, Icone }) => (
              <button
                key={id}
                onClick={() => setAba(id)}
                className={`flex flex-col items-center gap-0.5 py-2.5 text-[10px] font-bold ${
                  aba === id ? 'text-primary' : 'text-slate-500'
                }`}
              >
                <Icone size={20} />
                {rotulo}
              </button>
            ))}
          </div>
        </nav>
      </div>
    </div>
  )
}
